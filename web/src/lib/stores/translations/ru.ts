...
export const ru: I18nTexts = {
	common: {
		...
		clickToSelect:
			'Щёлкните по текстовому полю, чтобы выделить всё, или используйте кнопку копирования',
	...
	password: {
		...
		noLookAlikeNote:
			'Алфавит без путаницы исключает путающие символы. Минимум {0} символов для эквивалентной безопасности.',
		fullAlphabetNote:
	...
	apiKey: {
		...
		formatNotice:
			'Все API-ключи генерируются с префиксом "ak_" для лёгкой идентификации. Указанная длина относится только к генерируемым случайным символам (префикс не считается).',
		securityNotice:
			'Храните API-ключи безопасно и никогда не выставляйте их в клиентском коде или системе контроля версий. Обращайтесь с ними так же осторожно, как с паролями.',
