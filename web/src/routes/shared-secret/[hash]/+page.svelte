<script lang="ts">
	...
	function openDeleteDialog() {
		logger.info('[Click] Delete secret button clicked');
		showDeleteDialog = true;
	}

	async function confirmDelete() {
		logger.info('[Click] Delete confirmed');
		if (!hash) {
			flashMessagesStore.addMessage($_('sharedSecret.invalidHash'));
			setTimeout(() => goto('/'), 2000);
			return;
	...
	function cancelDelete() {
		logger.info('[Click] Delete cancelled');
		showDeleteDialog = false;
	}
...
<SimpleDialog bind:show={showDeleteDialog} title={$_('sharedSecret.confirmDelete')} size="md">
	{#snippet children()}
		<div class="space-y-4">
			<div class="flex items-start gap-3">
				...
				<div class="flex-1">
					<p class="text-gray-700 dark:text-gray-300">
						{$_('sharedSecret.confirmDeleteMessage')}
					</p>
					<p class="text-sm text-gray-500 dark:text-gray-400 mt-2">
						{$_('sharedSecret.confirmDeleteWarning')}
					</p>
				</div>
			</div>
		</div>
