...
function _page($$payload, $$props) {
  ...
  if (!lengthValid) {
    $$payload.out.push("<!--[-->");
    $$payload.out.push(`<p class="text-red-500 text-sm mt-1">${escape_html(store_get($$store_subs ??= {}, "$_", _)("common.length"))} ${escape_html(store_get($$store_subs ??= {}, "$_", _)("common.mustBeBetween"))} ${escape_html(minLength)} ${escape_html(store_get($$store_subs ??= {}, "$_", _)("common.and"))} 44</p>`);
  } else {
    $$payload.out.push("<!--[!-->");
  }
  ...
  if (store_get($$store_subs ??= {}, "$isLoading", isLoading)) {
    ...
    $$payload.out.push("<!--[!-->");
    $$payload.out.push(`${escape_html(store_get($$store_subs ??= {}, "$_", _)("password.generatePassword"))}`);
  }
