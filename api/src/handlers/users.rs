...
pub fn handle_users(
    ...
    if let Err(e) = initialize_database(env.clone()) {
        ...
        return Ok(Response::new(500, r#"{"error": "Database initialization failed"}"#));
    }
    ...
    match (method, path_parts.as_slice()) {
        ...
        (Method::Get, ["api", "users"]) => {
            handle_list_users(env, query_params)
        },
        
        // GET /api/users/:id - Get specific user
        (Method::Get, ["api", "users", id_str]) => {
            handle_get_user(env, id_str)
        },
        
        // POST /api/users - Create new user
        (Method::Post, ["api", "users"]) => {
            handle_create_user(env, req)
        },
        
        // DELETE /api/users/:id - Delete user
        (Method::Delete, ["api", "users", id_str]) => {
            handle_delete_user(env, id_str)
        },
        
        // Unsupported routes
        _ => {
            Ok(Response::new(404, r#"{"error": "User endpoint not found"}"#))
        }
    }
}
...
fn handle_get_user(env: DatabaseEnvironment, id_str: &str) -> anyhow::Result<Response> {
    ...
    match UserOperations::get_user_by_id(env, user_id) {
        Ok(Some(user)) => {
            Ok(Response::new(200, serde_json::to_string(&user)?))
        },
        Ok(None) => {
            Ok(Response::new(404, r#"{"error": "User not found"}"#))
        },
        Err(e) => {
...
fn handle_create_user(env: DatabaseEnvironment, req: Request) -> anyhow::Result<Response> {
    ...
    if user.username.is_empty() || user.email.is_empty() {
        return Ok(Response::new(400, r#"{"error": "Username and email are required"}"#));
    }
...
fn handle_delete_user(env: DatabaseEnvironment, id_str: &str) -> anyhow::Result<Response> {
    ...
    match UserOperations::delete_user(env, user_id) {
        Ok(true) => {
            Ok(Response::new(200, r#"{"message": "User deleted successfully"}"#))
        },
        Ok(false) => {
            Ok(Response::new(404, r#"{"error": "User not found"}"#))
        },
