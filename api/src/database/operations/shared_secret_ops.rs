...
impl SharedSecretOps {
    ...
    /// * `sender_id` - Pre-computed sender encrypted ID
    /// * `receiver_id` - Pre-computed receiver encrypted ID
    ///
    /// # Returns
    ...
    pub fn create_secret_pair(
        ...
        expires_hours: i64,
        max_reads: i64,
        sender_id: &[u8; ENCRYPTED_ID_LENGTH],
        receiver_id: &[u8; ENCRYPTED_ID_LENGTH],
        ...
        // Generate reference hash (same for both)
        ...
        // Encrypt payload for receiver (same payload, different key)
        let encrypted_payload_receiver =
            SharedSecretCrypto::encrypt_payload(receiver_id, &payload)?;

        // Calculate expiration timestamp in hours
        let expires_at = (Utc::now().timestamp() / 3600) + expires_hours;

        // Store sender entry (unlimited reads)
        ...
        // Store receiver entry (limited reads)
        SharedSecretStorage::store_shared_secret(
            receiver_id,
