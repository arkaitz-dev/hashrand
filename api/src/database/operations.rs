...
impl UserOperations {
    ...
    pub fn create_user(env: DatabaseEnvironment, user: &User) -> Result<i64, SqliteError> {
        ...
        Err(SqliteError::Io("Failed to get inserted user ID".to_string()))
    }
    ...
    pub fn get_user_by_id(env: DatabaseEnvironment, user_id: i64) -> Result<Option<User>, SqliteError> {
        let connection = get_database_connection(env)?;
    ...
    /// # Returns
    /// * `Result<Option<User>, SqliteError>` - User if found, None if not found, or database error
    pub fn get_user_by_username(env: DatabaseEnvironment, username: &str) -> Result<Option<User>, SqliteError> {
        let connection = get_database_connection(env)?;
    ...
    pub fn list_users(env: DatabaseEnvironment, limit: Option<u32>) -> Result<Vec<User>, SqliteError> {
        let connection = get_database_connection(env)?;
